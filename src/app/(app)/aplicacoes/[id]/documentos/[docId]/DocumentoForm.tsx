"use client";

import { useActionState } from "react";
import { updateDocumento } from "../actions";
import type { Documento } from "@/lib/types";

export function DocumentoForm({
  documento,
  aplicacaoId,
}: {
  documento: Documento;
  aplicacaoId: string;
}) {
  const [error, formAction, pending] = useActionState(updateDocumento, undefined);

  return (
    <form action={formAction} className="space-y-4 rounded-lg border border-border bg-surface p-6">
      <input type="hidden" name="id" value={documento.id} />
      <input type="hidden" name="aplicacao_id" value={aplicacaoId} />

      <div className="grid grid-cols-2 gap-4">
        <Field label="Código do Documento" name="codigo" defaultValue={documento.codigo} required />
        <Field
          label="Tipo de Documento"
          name="tipo_documento"
          defaultValue={documento.tipo_documento}
          required
        />
      </div>

      <Field label="Nome" name="nome" defaultValue={documento.nome} required />

      <div className="grid grid-cols-3 gap-4">
        <Field
          label="Data"
          name="data_documento"
          type="date"
          defaultValue={documento.data_documento ?? ""}
        />
        <Field label="Versão" name="versao" defaultValue={documento.versao ?? ""} />
        <Field
          label="Link de Acesso"
          name="link_acesso"
          type="url"
          defaultValue={documento.link_acesso ?? ""}
        />
      </div>

      {error && <p className="text-sm text-red-400">{error}</p>}

      <button
        type="submit"
        disabled={pending}
        className="rounded-md bg-gold px-4 py-2 text-sm font-medium text-background hover:bg-gold-soft disabled:opacity-60"
      >
        {pending ? "Salvando…" : "Salvar Alterações"}
      </button>
    </form>
  );
}

function Field({
  label,
  name,
  defaultValue,
  required,
  type = "text",
}: {
  label: string;
  name: string;
  defaultValue?: string;
  required?: boolean;
  type?: string;
}) {
  return (
    <div>
      <label htmlFor={name} className="block text-sm font-medium text-muted">
        {label}
      </label>
      <input
        id={name}
        name={name}
        type={type}
        required={required}
        defaultValue={defaultValue}
        className="mt-1 w-full rounded-md border border-border bg-surface-alt px-3 py-2 text-foreground focus:border-gold focus:outline-none"
      />
    </div>
  );
}
